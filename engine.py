...
def _business_days_lag(mx: date, expected: date) -> int:
    """Liczba DNI ROBOCZYCH (pon-pt) w (mx, expected]. 0 gdy mx >= expected.
    Piątek->poniedziałek = 1 (nie 3 kalendarzowe) — eliminuje fałszywe stale w pon."""
    if mx >= expected:
        return 0
    n = 0
    d = mx
...
def check_stale(expected: date) -> list[str]:
    """Lista przeterminowanych źródeł SCORE, licząc lag w DNIACH ROBOCZYCH z osobnymi
    progami: ceny CORE > STALE_CORE_MAX_BDAYS, makro FRED > STALE_FRED_MAX_BDAYS (większy
    lag publikacji). MAX(date) per symbol/serię BEZPOŚREDNIO z tabel (nie z source_health)."""
    stale = []
    with db.get_conn() as c:
        for s in config.CORE_SYMBOLS:
            mx = c.execute("SELECT MAX(date) FROM prices_eod WHERE symbol=?", (s,)).fetchone()[0]
            if mx is None or _business_days_lag(date.fromisoformat(mx), expected) > config.STALE_CORE_MAX_BDAYS:
                stale.append(f"price:{s}")
        for series in FRED_SCORE:
            mx = c.execute(
                "SELECT MAX(date) FROM macro_series WHERE series=? AND value IS NOT NULL", (series,)
            ).fetchone()[0]
            if mx is None or _business_days_lag(date.fromisoformat(mx), expected) > config.STALE_FRED_MAX_BDAYS:
